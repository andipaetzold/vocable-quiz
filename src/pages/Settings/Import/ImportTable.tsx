...
export default function ImportTable({ database }: Props) {
    ...
    const handleImport = async (userName: string, subjectName: string) => {
        ...
        const cardsToImport: Omit<Card, "id">[] = Karten.filter(karte => karte.Thema === subjectName).map(karte => ({
                ...
                <Table
                    ...
                    columns={[
                        ...
                        },
                        {
                            title: "Actions",
                            key: "action",
                            render: ({ user, subject }) => <Button onClick={() => handleImport(user, subject)}>Import</Button>
                        }
                    ]}
                />
            )}
